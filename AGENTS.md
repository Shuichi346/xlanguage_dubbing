...
- Keep Irodori-TTS-v4-Small in server API mode: use `Aratako/Irodori-TTS-v4-Small`, pass per-segment reference audio as `irodori.ref_wav`, keep the `8`-step Sway Sampling profile, do not send Caption / Style Prompt, and do not set fixed `seconds`.
