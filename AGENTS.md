...
- Keep Irodori-TTS-v4-Small in server API mode: use `Aratako/Irodori-TTS-v4-Small`, pass each speaker's cached long reference as `irodori.ref_wav`, keep the `8`-step Sway Sampling profile, do not send Caption / Style Prompt, and do not set fixed `seconds`.
- Limit Irodori long references to 120 seconds and compose them from multiple short utterances by the same speaker.
